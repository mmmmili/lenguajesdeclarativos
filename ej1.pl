...
rotar_turno_jugador -->
    state(S0, S),
    {
        ...
        enviar_puntaje_parcial(JugadoresRotados),
        S = [jugadores(JugadoresRotados), mesa(Mesa1), mazo(Mazo)]
    }.

enviar_puntaje_parcial(Jugadores) :-
    findall(Texto, (
        member(jugador(Nom, _, _, Ganadas, _, _), Jugadores),
        length(Ganadas, Cant),
        format(string(Texto), '~w: ~w cartas', [Nom, Cant])
    ), Lineas),
    atomic_list_concat(Lineas, ' | ', LineaFinal),
    format(string(MsgFinal), 'PUNTAJE: ~w', [LineaFinal]),
    enviar_a_todos(Jugadores, MsgFinal).
